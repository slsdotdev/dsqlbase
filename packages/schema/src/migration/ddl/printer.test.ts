...
describe("printDDL", () => {
  ...
  describe("ALTER_COLUMN", () => {
    ...
    it("adds identity GENERATED BY DEFAULT", () => {
      const node = ddl.alterTable({
        ...
        actions: [
          ddl.alterColumn({
            columnName: "id",
            actions: [ddl.addIdentity({ mode: "BY_DEFAULT" })],
          }),
        ],
      });
    ...
    it("adds identity GENERATED ALWAYS with options", () => {
      const node = ddl.alterTable({
        name: "users",
        actions: [
          ddl.alterColumn({
            ...
            actions: [
              ddl.addIdentity({
                mode: "ALWAYS",
                options: ddl.sequenceOptions({ startValue: 100, incrementBy: 1 }),
              }),
            ],
          }),
        ],
    ...
    it("adds a UNIQUE constraint USING INDEX", () => {
      const node = ddl.alterTable({
        name: "users",
        actions: [
    ...
    it("adds a PRIMARY KEY constraint USING INDEX", () => {
      const node = ddl.alterTable({
        name: "users",
        actions: [
          ddl.addConstraintUsingIndex({
