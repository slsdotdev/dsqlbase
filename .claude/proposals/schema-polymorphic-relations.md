...
0. **Prerequisites shared with Topic 1** (tracked separately): runtime primary key; `Table.alias` + shared `attachModels`; `$$meta` + resolver tree with per-level table context and a row-aware post-processing hook; codec-aware where; **table aliasing in lateral joins** (listed gap, needed by self-referential members); **composite relation pairs** (listed gap, branch predicates use every `from[i]` / `to[i]`).
